...
from data.dataset import one_hot_encoded
...
class MNIST:
    ...
    def convert_to_rgb(self):
        ...
        for i in range(self.num_train):
            if i < self.num_test:
                img_test = Image.fromarray(255 *  self.x_test[i].reshape(28, 28))
                rgb_img_test = img_test.convert('RGB')
                self.x_test_rgb.append(np.array(rgb_img_test))
            img_train = Image.fromarray(255 * self.x_train[i].reshape(28, 28))
            rgb_img_train = img_train.convert('RGB')
            self.x_train_rgb.append(np.array(rgb_img_train))

        self.x_train_rgb = np.array(self.x_train_rgb) / 255
        self.x_test_rgb = np.array(self.x_test_rgb) / 255
########################################################################